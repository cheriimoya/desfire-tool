...
pub fn change_key(
    aid: u8,
    kid: u8,
    mut key_old: [u8; 16usize],
    mut key_new: [u8; 16usize],
    version: Option<u8>,
) {
    ...
    let key_new =
        mifare::desfire_aes_key_new_with_version(&mut key_new[0], version.unwrap_or_else(|| 0) + 1);
    let key_old =
        mifare::desfire_aes_key_new_with_version(&mut key_old[0], version.unwrap_or_else(|| 0));
...
pub fn settings_struct_to_u8(settings: KeySettings) -> u8 {
    ...
    if settings.master_key_changeable {
        settings_byte |= 1 << 0;
    }
    if !settings.list_needs_master {
        settings_byte |= 1 << 1;
    }
    if !settings.create_needs_master {
        settings_byte |= 1 << 2;
    }
    if settings.settings_changeable {
        settings_byte |= 1 << 3;
    }
    if settings.change_needs_key_auth {
        settings_byte |= 0b00001110;
    }
    if settings.keys_frozen {
        settings_byte |= 0b00001111;
    }
    settings_byte
}

pub fn change_key_settings(
    aid: u8,
    kid: u8,
    mut key: [u8; 16usize],
    version: Option<u8>,
