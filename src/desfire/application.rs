...
pub fn add_application(application: Application, key: Option<[u8; 16usize]>, version: Option<u8>) {
    ...
    match application.encryption {
        Encryption::NONE => {
            res = mifare::desfire_create_application(
                desfire.tag,
                application.id,
                settings_struct_to_u8(application.settings),
                application.kid,
            )
        }
        Encryption::DES3K3 => {
            res = mifare::desfire_create_application_3k3des(
                desfire.tag,
                application.id,
                settings_struct_to_u8(application.settings),
                application.kid,
        ...
        Encryption::AES => {
            res = mifare::desfire_create_application_aes(
                desfire.tag,
                application.id,
                settings_struct_to_u8(application.settings),
                application.kid,
