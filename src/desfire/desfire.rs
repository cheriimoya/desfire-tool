...
impl DesFire {
    pub fn initialize() -> DesFire {
        ...
        for i in 0..device_count {
            let device =
                unsafe { nfc_sys::nfc_open(&mut to_sys_context(*context), devices.as_mut_ptr()) };
    ...
    pub fn connect(&self) {
        ...
        if res < 0 {
            panic!("cannot connect to tag!")
        }
        let mut info = freefare_sys::Struct_mifare_desfire_version_info {
            ..Default::default()
        };
        let res = mifare::desfire_get_version(tag, &mut info);
        if res < 0 {
            panic!("cannot read tag version!")
        }
        if info.hardware.version_major < 1 {
            panic!("Tag older than EV1!")
        }
        unsafe {
            println!(
                "Found tag {} with UID {}",
                *freefare::get_tag_friendly_name(tag),
                *freefare::get_tag_uid(tag)
            );
        }
    }
    ...
    pub fn authenticate(&self, key_id: u8, mut key: [u8; 16usize], version: Option<u8>) {
        let key =
            mifare::desfire_aes_key_new_with_version(&mut key[0], version.unwrap_or_else(|| 0));
